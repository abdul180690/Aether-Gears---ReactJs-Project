    ...
    <nav className={`${containerStyles}  flex gap-4`}>
      {navLinks.map((link) => (
        <NavLink
          ...
          to={link.path}
          className={({ isActive }) =>
            `${isActive ? 'active-link ' : 'text-gray-900'} flex items-center px-3 py-2 rounded-full hover:bg-primary-light hover:bg-slate-800 hover:text-primary  transition-all`
          }
          onClick={onClick}
        >
